...
export function DashboardClient() {
  ...
  const { kpis, porTime, porConsultor, serieMensal, consultores, times, ultimaColeta } = data ?? {
    kpis: { total: 0, concluidos: 0, emAberto: 0, slaPercent: 0, tmaMedio: null },
    porTime: [], porConsultor: [], serieMensal: [], consultores: [], times: [], ultimaColeta: null,
  };
    ...
    <div className="space-y-6">
      ...
      {porTime.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {porTime.map(t => (
            <div key={t.time} className="bg-white rounded-xl border-2 shadow-sm p-5"
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-gray-900 text-sm">Time {t.time}</h3>
                </div>
                <span className="text-xs text-gray-400">{t.total.toLocaleString('pt-BR')} tickets totais</span>
              </div>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <p className="text-2xl font-bold text-green-600">{t.concluidos.toLocaleString('pt-BR')}</p>
                  <p className="text-xs text-gray-400 mt-0.5">Concluídos</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">SLA</p>
                </div>
              </div>
            </div>
          ))}
        ...
        return (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <div className="flex items-start justify-between mb-5">
              <div>
                <h2 className="text-sm font-semibold text-gray-700">Concluídos por Mês</h2>
                <p className="text-xs text-gray-400 mt-0.5">Últimos 12 meses · linha tracejada = média ({media}/mês)</p>
              </div>
              <div className="flex items-center gap-6 text-right">
                <div><p className="text-2xl font-bold text-green-600">{mesAtual.toLocaleString('pt-BR')}</p><p className="text-xs text-gray-400">mês atual</p></div>
                <div><p className={`text-lg font-semibold ${variacao >= 0 ? 'text-green-600' : 'text-red-500'}`}>{variacao >= 0 ? '↑' : '↓'} {Math.abs(variacao)}%</p><p className="text-xs text-gray-400">vs anterior</p></div>
                <div><p className="text-lg font-semibold text-gray-700">{total12.toLocaleString('pt-BR')}</p><p className="text-xs text-gray-400">total 12m</p></div>
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-gray-700">Ranking por Time e Consultor</h2>
          <p className="text-xs text-gray-400 mt-0.5">Clique no consultor para ver detalhes · Clique no time para expandir/recolher</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="text-right px-4 py-3 font-medium text-gray-500 text-xs uppercase hidden xl:table-cell">TMA</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(consultoresPorTime).map(([time, consultoresDoTime]) => {
                ...
                return [
                    ...
                    <td colSpan={8} className="px-6 py-3 border-t-2" style={{ borderColor: cor }}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-bold text-gray-900 text-sm">Time {time}</span>
                          <span className="text-xs text-gray-400">· {consultoresDoTime.length} consultores</span>
                        </div>
                        <div className="flex items-center gap-6 text-right text-sm">
                          <span className="text-gray-500">{totalTime.toLocaleString('pt-BR')} tickets</span>
                  ...
                  ...(isOpen ? consultoresDoTime.map((c, idx) => (
                    <tr key={c.consultor} className="hover:bg-gray-50 transition-colors border-b border-gray-50">
                      <td className="px-6 py-3.5 pl-12 text-gray-400 text-xs font-medium">{rankOffset + idx + 1}</td>
                      <td className="px-4 py-3.5">
                        <Link href={`/consultor/${encodeURIComponent(c.consultor)}`}
                          className="font-medium text-blue-600 hover:underline">
                          {c.consultor.replace(/\s*\(N\d+\)$/, '')}
                        </Link>
                        <p className="text-xs text-gray-400 mt-0.5">{c.grupo}</p>
                      </td>
                      <td className="px-4 py-3.5 text-right text-gray-600 tabular-nums">{c.total.toLocaleString('pt-BR')}</td>
      ...
      {porConsultor.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Evolução de Concluídos — Top 5 Consultores</h2>
          <ResponsiveContainer width="100%" height={240}>
